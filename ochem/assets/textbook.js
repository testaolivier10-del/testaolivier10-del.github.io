...
(function(){
  ...
  // ---- one chapter ------------------------------------------------------
  var NOTES_START = '<!-- notes:start -->';
  var NOTES_END = '<!-- notes:end -->';
  var notesCache = {};
  function loadNotes(id){
    ...
    notesCache[id] = fetch('notes/' + id + '.html', { credentials: 'omit' })
      ...
      .then(function(html){
        // Each notes file is now a standalone page as well as the source this
        // fetches — it has its own head, navigation and onward links, so that
        // the course's prose is readable with JavaScript off and indexable by
        // a crawler that does not run scripts. The prose itself sits between
        // two markers; everything outside them is that page's shell, which
        // this view already provides and must not inject twice.
        // Generated by scripts/build-notes-pages.mjs.
        var a = html.indexOf(NOTES_START);
        var b = html.indexOf(NOTES_END);
        if(a !== -1 && b !== -1) html = html.slice(a + NOTES_START.length, b);
