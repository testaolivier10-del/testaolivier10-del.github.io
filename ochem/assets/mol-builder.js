...
(function(){
  ...
  /* ---- A structure that fits in a link -----------------------------------

     Typed formulas travel fine as text, but a structure somebody DREW has no
     text form, and the product of a mechanism has none either — which is
     exactly the molecule you would want to send to the 3D viewer. So:

       C.O-1.C|0-1:1,0-2:2

     atoms as element plus optional charge and a pinned hydrogen count, then
     bonds as index pairs with an order. Coordinates are left out and
     regenerated by layout() at the other end, because where the atoms sat on
     someone else's canvas is not information worth carrying.

     Not a standard format and not pretending to be one. It is short, it
     survives a URL without escaping, and both ends of it are in this file. */
  function encode(st){
    var keys = Object.keys(st.atoms).filter(function(k){
      var a = st.atoms[k];
      return a.el && !a.group;
    ...
    var index = {};
    var atoms = keys.map(function(k, i){
      index[k] = i;
      var a = st.atoms[k];
      return a.el +
        (a.charge ? (a.charge > 0 ? '+' + a.charge : String(a.charge)) : '') +
        (a.hFixed !== undefined ? 'h' + a.hFixed : '');
    }).join('.');

    var bonds = st.bonds.filter(function(b){
      return b.order > 0 && index[b.a] !== undefined && index[b.b] !== undefined;
    }).map(function(b){
      return index[b.a] + '-' + index[b.b] + (b.order > 1 ? ':' + b.order : '');
    }).join(',');
  ...
  function decode(text){
    try{
      ...
      var st = blank();
      parts[0].split('.').forEach(function(tok, i){
        var m = /^([A-Z][a-z]?)([+-]\d+)?(?:h(\d+))?$/.exec(tok);
        if(!m) throw new Error('bad atom');
        var a = {
          el: m[1], label: m[1], group:false,
          x:0, y:0, r: m[1] === 'H' ? 12 : 18,
          lp:0, charge: m[2] ? parseInt(m[2], 10) : 0, hImplicit:0
        };
        if(m[3] !== undefined) a.hFixed = parseInt(m[3], 10);
        st.atoms['a' + (i + 1)] = a;
      });
      (parts[1] || '').split(',').forEach(function(tok){
        if(!tok) return;
        var m = /^(\d+)-(\d+)(?::(\d+))?$/.exec(tok);
        if(!m) throw new Error('bad bond');
        st.bonds.push({
          a: 'a' + (parseInt(m[1], 10) + 1),
          b: 'a' + (parseInt(m[2], 10) + 1),
          order: m[3] ? parseInt(m[3], 10) : 1
        });
      });
      if(!Object.keys(st.atoms).length) return null;
      layout(st);
      normalize(st);
      return st;
    }catch(e){ return null; }
