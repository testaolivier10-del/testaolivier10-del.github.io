...
ochem/                 The Organic Chemistry course
  ...
  assets/
    ...
    flashcards.json        Generated by scripts/build-flashcards.mjs from the notes'
                            tables and scripts/lib/flashcard-sources.mjs. Do not edit
    flashcard-scheduler.js When a flashcard comes back (SM-2, four self-grades)
    flashcards-page.js     flashcards.html: the deck view, filters, session and input
...
Lesson progress is stored in `ochem_progress` (per-topic `{correct, attempts}`, read by `curriculum.js`); the concept model lives in `ochem_mastery_v1` (see `ochem/assets/mastery-engine.js`); the game layer's own state — concept badges, daily Rounds, achievements — lives in `ochem_game_v1` (`ochem/assets/ochem-xp.js`); and the flashcard schedule lives in `ochem_flashcards_v1`. All four sync with an account.
...
### Flashcards (`ochem/flashcards.html`)

A spaced-repetition deck — 663 cards — that is not a second copy of the course. Three sources, two of which the course already had:

- **Concepts (137).** One card per concept in `concepts.js`: its title on the front, its one-line `hint` and the micro-lesson from `concept-teach.json` on the back. Assembled in the browser by `flashcards-page.js` from the same two files Practice teaches from, so a corrected teach string corrects its card with no rebuild.
- **Facts from the notes' tables (494).** pKa values, IR bands, ¹H and ¹³C shifts, mass-spec losses, reagent → product menus, directing effects, the comparison tables (SN2 vs E2, kinetic vs thermodynamic enolate…). `scripts/build-flashcards.mjs` reads them out of `ochem/notes/*.html` by the rules in `scripts/lib/flashcard-sources.mjs` — which table, which column is the question, how it is asked — and writes `ochem/assets/flashcards.json`. A rule finds its table by the header row and **fails the build** if the header no longer matches, so a restructured table gets looked at instead of turning into nonsense cards; `--check` runs in CI, so a value corrected in the notes cannot stay wrong on its card.
- **Named reactions and rules (32).** The ones the notes teach in prose, so there is no cell to read: Wittig, Claisen, Robinson, Baeyer–Villiger, Hofmann, haloform, Hückel's rule and so on. Authored in that same sources file, each against the section it names.

Every card belongs to a curriculum topic, which gives it its chapter and a link back to the section (and lesson) that teaches it. Filters: chapter; card type; and **Studied** / **Needs work**, which read what the course already records — sections read, lesson runs, the concept model under 70%, unresolved missed questions, and cards you keep forgetting.

Scheduling (`flashcard-scheduler.js`) is SM-2 with Again/Hard/Good/Easy: new cards step 1 day, then 3, then × ease; Again is back in ten minutes, later in the same session; intervals land at local midnight and cap at a year; twenty new cards a day. **Studying ahead never pushes a card further out** — only an Again changes a card that is not due. **Flashcards never move a mastery number**: a self-grade is weaker evidence than an answer the engine checked. XP is 2 per distinct card reviewed on schedule, whatever the grade (paying more for Good would pay for pressing it), capped at 60 a day, and it keeps the streak.
...
`sources.html` promised a way to tell us when a question is wrong from the day it was written. It explained the correction policy and said where corrections get listed, and then never said *how* — the only address anywhere on the site was at the bottom of the privacy policy. For a bank of 2,106 NREMT questions and 3,630 ochem ones, written against reference material rather than by a committee, that was the most expensive gap on the site. No script can check whether an answer is clinically right; a student who has just answered one and thinks the key is wrong is the only reviewer who can, and they are on the one screen where saying so costs a tap.
