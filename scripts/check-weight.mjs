...
const BUDGETS = [
  ...
  ['index.html', 6],
  ['nremt/index.html', 9],
  // 10 -> 11. The home page carries one JSON-LD LearningResource entry and one
  // curriculum row per topic, both generated by build-ochem-home.mjs, so every
  // section added to the course adds about 200 bytes of it. At 117 topics the
  // page measured just inside 10 KB gzipped and the next topic put it over.
  // The budget follows the course rather than the other way round; the saving
  // noted above — splitting lesson-concepts.js out of ochem-home.js — is still
  // available if this needs to come back down.
  ['ochem/index.html', 11],
