...
const TOPICS=['oxidation-states','alcohol-oxidation','carbonyl-reduction','hydrogenation','alkene-oxidation'];
let bad=0;
for(const t of TOPICS){
  const page=await b.newPage(); const errs=[]; page.on('pageerror',e=>errs.push(String(e)));
  await page.goto(`http://localhost:8792/ochem/lessons/${t}.html`);
  await page.waitForFunction(()=>document.querySelector('#card .step-title'));
  let axeRun=false;
  for(let g=0;g<60;g++){
    ...
    for(let pass=0;pass<6;pass++){
      ...
      for(const btn of btns){ await btn.click().catch(()=>{}); await page.waitForTimeout(25);
        if(await page.$('#nextBtn:not([disabled])')) break; }
      if(await page.$('#nextBtn:not([disabled])')) break;
    }
    const ch=await page.$$('#card .choice-row .choice-btn');
    if(ch.length){
      for(const c of ch){ await c.click().catch(()=>{}); await page.waitForTimeout(25);
  ...
  if(!ok) bad++;
  console.log(`${ok?'OK  ':'FAIL'} ${t.padEnd(20)} completed=${!!(st.prog&&st.prog.completed)} concepts=[${st.concepts}] errors=${errs.length||'none'}`);
  await page.close();
}
console.log(bad? `\n${bad} PROBLEM(S)` : '\nall five redox lessons complete cleanly, axe clean');
