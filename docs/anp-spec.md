...
* Exception, everyday body words (decision 51): common words any student already roughly knows (rib, nerve, cartilage, fever, gland, hormone and similar, listed under everydayWords in the dependency map) may be used in their everyday sense before their full lesson, with a glossary hover that links to where they are taught. Technical concepts stay strict.
...
Phase 1: Pilot. Build Foundations plus the full cardiovascular chapter using every standard in this spec: page template, writing standards, visual language, concept ordering and the build check, and working versions of every tool (lab practical, predict the change, feedback loop builder, pathway tracer, graph reader, calculators, word root builder, flashcards) with cardiovascular content. Include the question bank for these topics, the course home page, the cardiovascular chapter page, and the relevant dashboard pieces. Run the audit and all site checks. Show me what you built, anything flagged for human review, and any spec decisions you had to make. Mark the course as Beta on the site. (Built 2026-09-23 and waiting for the owner's review; see docs/anp-phase1-report.md.)
STOP for my review.

Phase 2: Respiratory and nervous systems, then the rest of A&P II, then the TEAS mode. Publish each system as it passes the audit. No review stop: give a short summary after each chapter and continue. (Done 2026-09-24: every A&P II chapter published; TEAS mode serves the full 18 questions.)

Phase 3: The rest of A&P I and both cumulative finals. No review stop: give a short summary after each chapter and continue. (Done 2026-09-24: all 27 chapters and 162 topics published, both cumulative finals verified; see docs/anp-phase3-report.md.)
...
OpenStax coverage recheck (2026-09-23), run without a review stop per decision 13:
14. Method: every section's learning objectives (169 sections) and every chapter's key terms (3,190) were pulled from the live OpenStax A&P 2e site. 1,610 key terms matched a map term or alias directly; the other 1,580 were sorted by five reviewers, one per chapter group, into aliases, new concepts, or out of scope. Result: 39 new concepts, about 1,200 new aliases, 453 terms skipped as lab-image detail or plain words. All additions fit existing topics.
15. Germ layers at the tissues chapter (OpenStax 4.1 names them): not pulled forward. The tissue topics do not need them to make sense; the embryonic development topic ties each germ layer back to the tissues it forms. Reason: a preview box would add a forward reference that teaches nothing the tissue pages use.
16. Brain embryology: OpenStax 13.1 teaches the neural tube and brain vesicles with the brain. A short "brain development (basic)" concept is pulled forward into Regions of the brain and owns those terms; embryonic development keeps the full treatment. Reason: the spec's default fix for a circular dependency is a short pulled-forward version, not a preview box.
17. Stroke in the neurological exam and the enteric nervous system in the autonomic chapter: no preview boxes. Both can be written in plain words where they first appear ("loss of blood flow to part of the brain"; "the gut's own nerve network"), and each is taught fully later.
18. Musculoskeletal embryology (somites, limb buds) and anabolic steroids: taught where their prerequisites are (embryonic development; other endocrine organs). The skeleton, bone growth and fiber type pages do not use those terms. No preview boxes.
...
Phase 1 build decisions (2026-09-23):
25. Architecture: docs/anp-phase1-architecture.md. The course is data-driven: authors write anatomy-physiology/data/ (lesson JSON, notes HTML fragments, per-topic question banks, per-topic glossary and figure files), and scripts/build-anp.mjs generates every lesson, notes, chapter, core concept and glossary page with a --check in CI. Reason: the spec's fixed 11-part lesson template and the ordering check both need every page built the same way from data.
26. Shared vs own engines: A&P reuses the site-wide modules as they are (header, theme, XP/levels/streaks/goals, accounts and sync, Report a problem, analytics, service worker, search ranking, flashcard scheduling) and registers with each as a third course, key "anp". It does not reuse ochem's lesson, question and mastery engines: they are wired to chemistry concepts, the molecule renderer and ochem progress keys, and generalizing them would mean rewriting reviewed ochem code (an early-stop condition). A&P gets its own anp-*.js modules built on the same ideas (concept strength with decay, SM-2 review, tiered XP).
27. Anatomy panels and the lab practical use OpenStax figures (commercially cleared ones only, decision 22) with their baked-in labels covered by mask boxes. Hide-labels, tap-to-reveal and the quiz modes all work on the real figures. The mask boxes are found automatically in the figures' white margins and checked by eye. Physiology diagrams, graphs and loops are our own SVG in the shared visual language.
28. Content is written by parallel authors per chapter group against docs/anp-authoring-guide.md, each topic checked by scripts/check-anp-content.mjs (formats, question rules, licenses, and the ordering rule applied to lesson, notes, questions and glossary text), then audited by a second, skeptical reviewer before it is published (section 16).

29. Chapter pages exist only for chapters with at least one built topic. The course home and "All chapters" list unbuilt chapters as plain text ("coming in a later part"), with no link. Reason: decision 1 rules out "coming soon" pages, and an empty chapter page would be one.
30. Figure labels that name a later concept are covered for good on that page. A figure's named labels carry the concept they name; on a page for topic N, a label whose concept is taught after N is painted over (not revealable), on lesson anatomy panels, notes figures and figure questions alike. Reason: the ordering rule (section 7) applies to words printed on a figure as much as to the text; the page check cannot read pixels, so the generator enforces it from the label data.
31. Navigation and labels are not teaching. Core-concept tags, "builds on", "what comes next" and previous/next links sit in elements marked anp-nav-ref, which the page check skips (as it skips preview boxes). Reason: a tag reading "Homeostasis" on a chemistry page points to the core concept; it does not use the term.
32. Page-check matching: a term that starts with a single capital letter as its own word ("A band", "Z disc", "T tubule") is matched case-sensitively, and sub/superscripts are joined to their word before scanning (CO<sub>2</sub> reads "CO2", not "CO", cardiac output). Reason: both produced false failures on correct pages.
33. Wide tables: the generator wraps every table in a scrolling wrapper so it scrolls inside itself on a phone (site-wide rule).
34. Lesson and notes titles: when a title is too long for 60 characters, the kind ("A&P lesson:", "A&P notes:") comes first, so the two pages for one topic never share a truncated title.
35. Figure credits page (anatomy-physiology/credits.html) lists every OpenStax figure on a built page with its figure number, source page and license, and every A&P footer links to it. sources.html has an A&P section with the OpenStax attribution, the non-endorsement statement and the TEAS disclaimer. Reason: section 2 asks for each figure's license and credit on record, and the per-figure caption credit alone is hard to audit.
36. Originality audit: scripts/audit-anp-originality.mjs compares every sentence of the course with the saved OpenStax section pages and lists shared runs of 9+ words for a person to read. It is local (the reference text is not in the repo). First run over 29 topics: 9 runs, all standard anatomical phrasing ("the right ventricle receives blood from the right atrium through the tricuspid valve").
37. Tool content is validated in the content check: each data/tools/<slug>.json must have a validator at scripts/lib/anp-tool-checks/<slug>.mjs (docs/anp-tools-contract.md).
38. Plain-word paraphrases forced by the ordering rule in Foundations ("the chest's bony cage" before ribs are taught, "gristle" for chondr/o) are accepted for the pilot rather than adding preview boxes. Reason: none of those words is needed to understand the topic, and a preview box for each would clutter the first chapter; the reviewer is asked whether any should become a preview (docs/anp-needs-author.md).
39. Flashcards pay XP and count toward the streak, and keep their own spaced schedule (anp_flashcards_v1, merged card by card on sync), but do not change mastery or feed the question review queue. Reason: a grade the learner gives themselves is weaker evidence than a graded answer (the Organic Chemistry course's rule), and a card id in the review queue would be an item review cannot serve. This is a deliberate exception to section 8's "every tool updates mastery"; every other tool records graded items through AnpCore.
40. "Receptor" is enforced by the content check: "receptor" straight after a determiner ("the receptor", "its receptors") fails unless it reads "receptor protein" or "Receptor (sensor)". Named receptor proteins ("beta-1 receptors", "intracellular receptor") pass.
41. Core concept pages carry a short authored introduction each (anatomy-physiology/data/core-concepts.json), with examples from the built chapters.
42. Exams record every answer when the exam ends, not as it is answered, because the XP toast after each recorded answer revealed right and wrong mid-exam. An unanswered question is recorded as a miss so it reaches review.
43. TEAS mode in a partial course: one question per area that has built content plus round(6 × built areas / 12) rotating extras, stated on the page with the list of included areas; it reaches 18 when all 12 areas are built. Prediction items are left out (not a TEAS format). Timing 1.2 minutes per question; other exams 1.5.
44. Our own diagrams can be registered as figures, so a graph or image question can show one: `"source": "levlprep"`, `"license": "LevlPrep original"`, `"ext": "svg"`, file at anatomy-physiology/figures/<id>.svg, credit "LevlPrep". The SVG follows the visual language but is a standalone file (so it cannot use the page's CSS classes; it carries its own colors, taken from the anp.css tokens). The content check enforces all of this. Reason: the cardiovascular writers could not give the Frank–Starling curve a graph question because only OpenStax images could be registered.
45. The exercise prediction theme moves from Cardiac output to Short-term blood pressure control, where the exercise response and local autoregulation are taught. The feedback loop builder uses spec 8.3's seven slots (with afferent and efferent pathways); childbirth is its only positive loop, since clotting does not fit the seven slots and belongs to the blood chapter.
46. Figure labels are also covered when their printed name uses a later term, even if the label's concept is earlier or unmapped (for example "postsynaptic neuron" on a synapse figure in Chemical signaling). The generator runs the page check on each label name (extends decision 30).
47. Calculators in the pilot: flow and resistance, the radius rule, stroke volume and ejection fraction, pulse pressure, MAP, organ flow, cardiac output, net filtration pressure and total peripheral resistance. The acid–base (blood gas) calculator comes with the acid–base disorders topic, outside the pilot.
48. A figure label may be marked "cover": true in its labels file when the label printed on the OpenStax figure is wrong or misleading (first case: Figure 3.17 prints "Plasma membrane" on the peroxisome's own membrane). It is covered on every page and never asked in the lab practical, and the caption explains it.
49. Osmosis is taught as bulk flow of water through membrane pores, driven by the lower water pressure the held-back solute creates on the concentrated side; "water follows solute" is the working rule, and "water diffuses down its own concentration gradient" appears only in a "For your exam" note (docs/anp-needs-author.md, osmosis-mechanism).
50. The ordering check also matches the regular plural of every term ("keratins", "salivary glands", "muscle cells"), unless the map already lists it; before this, a plural slipped past. The map alias "colloid" (thyroid) becomes "thyroid colloid", because "colloid" is also the general chemistry word taught in Water and solutions.

Owner review of the Phase 1 pilot (2026-09-23): pilot approved and merged to main.
51. Everyday body words. Common words any student already roughly knows may be used in their everyday sense before the topic that teaches them: rib, nerve, cartilage, tendon, ligament, clot, hormone, gland, inflammation, hair, nail, sweating, shivering, bone marrow, hip bone, thirst, pus, fever, vaccine, allergy, saliva, mucus, bile, colon, rectum, puberty, menstruation, red and white blood cells, platelets, rib cage, nerve cell, heart attack, serum, muscle cell (and words like muscle, bone, skin, heart, lung and blood, which were never restricted). The list lives in docs/anp-dependency-map.json under everydayWords; the page check skips those exact words and their plurals, and the glossary gives each one a hover definition that links to where it is taught once that page exists. Technical terms stay strict, including technical phrases built on an everyday word ("skeletal muscle", "spinal nerve", "true ribs"). Foundations pages were rewritten to use natural wording where the old rule forced paraphrases. Supersedes decision 38.
52. Heart chambers and great vessels move into the first heart lesson (Heart position and coverings): the four chambers and the great vessels (venae cavae, pulmonary trunk, arteries and veins, and the aorta with its ascending part and arch) are taught there, so its figure shows those labels. Valves, septa, the cardiac skeleton and the detailed internal structure stay in Heart chambers and valves.
53. Publishing by chapter: anatomy-physiology/data/published.json lists the chapters that are live. A topic's pages, bank questions and tool items appear only when its files exist and its chapter is listed. A chapter is added once it passes the two-stage audit and every check, and then the branch is merged to main. Reason: the owner asked for each finished chapter to be merged to main as it passes, while later chapters are still being written on the same branch; this keeps unaudited work off the live site without juggling branches.
54. With decision 53, `check-anp-content.mjs --check` fails CI only for topics in published chapters; problems in a chapter still being written are printed but do not block. The glossary file carries every written definition, published or not (so early pages can show hovers for later terms), so its weight budget rises with each chapter.
55. Surfactant becomes its own concept, taught in Lower airway and lungs where the type II alveolar cells that make it are taught; lung compliance (the former "lung compliance and surfactant" concept) stays in Ventilation mechanics and builds on it.
56. Identification questions: an image question's figure covers every printed label and highlights the pinned label's box as the target, on practice, review, lessons and exams. Before this, the pin was never drawn and the answer could be read off the figure. The content check now requires every image question's pin to be a named label on its figure.
57. More everyday words (extends decision 51): reflex, pupil, eyelid and taste bud may be used in their everyday sense before their topics. Reason: the peripheral-nerves writers had to describe the blink and gag tests without the word "reflex" and the eye without "pupil" or "eyelid", which no student needs taught.
58. Tool data is published by chapter too (extends decision 53): the build writes `anatomy-physiology/assets/tool-data/*.json`, copies of the tool files that keep only items whose topic is built (lab figures, calculator groups and word parts trimmed to match), and the tool pages and flashcards read those copies. Reason: the tools read `data/tools/` directly, so items for chapters still in audit showed on the live tools once the branch merged to main. `data/tools/` stays the single source the content check validates. The site's spelling check now also sees tool text, which caught two British spellings.
59. The question bank is split by chapter: `assets/bank/<chapter>.json` and `<chapter>-why.json`, loaded with `AnpCore.loadBank`, each file with its own weight budget (48 KB core, 66 KB explanations). Reason: the single bank reached 174 KB and 247 KB gzipped with 72 topics and would pass 400 KB for the full course, and every chapter's edit invalidated the whole file in students' caches. The service worker no longer precaches the bank; files are cached as they are fetched.
60. The ordering check also matches the singular of a multi-word plural term ("carotid bodies" catches "carotid body", "plasma proteins" catches "plasma protein"). Single words are left alone, since stripping an s from one word misfires. It caught eight early uses of later terms, reworded in plain words (for example "a protein in the plasma" before the blood chapter).
61. "Ovarian cycle" becomes its own concept, taught in Oogenesis and the ovarian cycle (follicle development, ovulation, corpus luteum); it was an alias of "hormonal control of the ovarian cycle", taught one topic later, so the ordering check failed on the topic's own title. The hormonal-control concept keeps "follicular phase" and "luteal phase" and now depends on the new concept.
62. The milk ejection reflex is taught as a neuroendocrine reflex, not a positive feedback loop: suckling triggers oxytocin release, but ejecting milk does not feed back to strengthen the stimulus. OpenStax 28.6 calls it positive feedback, so the postnatal-lactation notes give that as the "For your exam" answer. In the tools it is a pathway, and the dependency map moves it from the development chapter's feedback loops to its pathways. Labor (oxytocin and uterine stretch) stays the chapter's positive feedback loop.
63. The built glossary (glossary page, hover definitions, glossary.json) includes only concepts taught in published chapters. Before, every definition in data/glossary went out, so a chapter still being written or audited leaked unaudited definitions to the live site, and its writers' edits kept the build check stale. Hovers for a later chapter's terms appear once that chapter is published.
64. Map fixes for words a later concept owned (dependency map only; no page text changed). A skeleton or muscle page could not print these words because a later topic's concept owned the bare form, so writers reworded or covered correct figure labels. Where one word names two different things, both concepts now carry a qualified label, so the page check leaves the bare word alone (the rule in docs/anp-phase0.md); where it names one structure, the earlier topic owns it. (1) "Foramen ovale": the fetal heart opening becomes "foramen ovale (heart)" beside the skull's "foramen ovale (skull)". "Nasal septum" is one structure, so the skull landmarks own the bare word and the upper airway's alias becomes "nasal septum (airway)". (2) Spinal nerve rami: bare "ramus" and "rami" are replaced by "ramus (nerve)", "dorsal ramus", "ventral ramus", "dorsal rami", "ventral rami" and "rami communicantes", so the mandible's ramus and the ischiopubic ramus can be printed. (3) "Satellite cell" becomes "satellite cell (glia)" beside "satellite cell (muscle)", and the muscle concept adds "muscle satellite cell". (4) Antibody "heavy chain" and "light chain" become "heavy chain (antibody)" and "light chain (antibody)", with "antibody heavy chain" and "antibody light chain", so myosin's heavy and light chains can be printed. (5) The growth plate concept adds "maturation and hypertrophy", the zone's printed label, which "hypertrophy" (fiber types) had covered. (6) Bone markings add "protuberance", plus the display labels "canal (bone)", "sinus (bone)" and "fissure (bone)": bare "canal" is an everyday word earlier pages use ("ear canal" in Hair, nails and glands), bare "sinus" names paranasal, venous, renal and lymph node sinuses too, and bare "fissure" is already qualified for the lung. "Sulcus", "foramen" and "fovea" are left with their owners (cerebrum, skull openings, photoreceptors), because moving them would change another concept's words. Trade-off: a qualified word gets no hover definition, so bare "foramen ovale", "satellite cell" and "light chain" on later pages show none.
65. The glossary file stays one file and its weight budget rises from 130 to 150 KB. It is fetched only when a hover or a glossary-based tool needs it, and the word-roots, search and flashcards tools read all of it, so splitting it by chapter (as the question bank was, decision 59) would add requests without saving bytes for those tools. The whole course is projected at about 138 KB.

Phase 3 close-out (2026-09-24):
66. With all 27 chapters published, the partial-course wording steps aside instead of being deleted: the course home says every topic is built, the finals describe their split rather than "what this final covers today", the glossary drops "so far", the practice page drops the pilot line, and TEAS mode shows no "scaled" note (decision 43's scaling still applies if a chapter is ever unpublished). Also fixed: when a small pick left one question to place (each TEAS area's single question, a small chapter's share of a final), the level tie always went to recall, so all 12 one-per-area TEAS questions were recall; ties now go to a level drawn by the target mix. Reason: the pages should read naturally for a finished course, and the TEAS set should carry the same level mix as the other exams.
